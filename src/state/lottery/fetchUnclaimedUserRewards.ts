...
const fetchCakeRewardsForTickets = async (
  ...
  try {
    ...
    const cakeTotal = cakeRewards.reduce((accum: BigNumber, celebrityReward: EthersBigNumber[]) => {
      return accum.plus(new BigNumber(celebrityReward[0].toString()))
    }, BIG_ZERO)

    const ticketsWithUnclaimedRewards = winningTickets.map((winningTicket, index) => {
      return { ...winningTicket, celebrityReward: cakeRewards[index] }
    })
