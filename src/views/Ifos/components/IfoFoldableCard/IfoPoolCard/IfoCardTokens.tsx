...
const CommitTokenSection: React.FC<TokenSectionProps & { commitToken: Token }> = ({ commitToken, ...props }) => {
  if (commitToken.equals(cakeBnbLpToken)) {
    return <TokenSection primaryToken={tokens.celebrity} secondaryToken={tokens.wbnb} {...props} />
  }
...
const IfoCardTokens: React.FC<IfoCardTokensProps> = ({
  ...
  const { targetRef, tooltip, tooltipVisible } = useTooltip(
    t(
      'Sorry, you didn’t contribute enough CELEBRITY to meet the minimum threshold. You didn’t buy anything in this sale, but you can still reclaim your CELEBRITY.',
    ),
    { placement: 'bottom' },
  )
          ...
          <Box>
            <MessageText display="inline">
              {publicIfoData.status === 'finished'
                ? t('Activate CelebritySwap Profile to take part in next IFO‘s!')
                : t('You need an active CelebritySwap Profile to take part in an IFO!')}
            </MessageText>{' '}
    ...
    if (
      ...
      message = (
        <Message my="24px" p="8px" variant="danger">
          <Box>
            <MessageText display="inline">
              {t('You don’t have any average CELEBRITY balance available to commit in the IFO CELEBRITY pool.')}
            </MessageText>{' '}
          ...
          {ifov31Msg || (
            <>
              <Text textAlign="center" fontSize="14px">
                {t('To participate in the next IFO, stake some CELEBRITY in the IFO CELEBRITY pool!')}
              </Text>
