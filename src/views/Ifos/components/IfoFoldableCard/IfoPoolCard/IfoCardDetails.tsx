...
const MaxTokenEntry = ({ maxToken, ifo, poolId }: { maxToken: number; ifo: Ifo; poolId: PoolIds }) => {
  const isCurrencyCake = ifo.currency === tokens.celebrity
  ...
  const basicTooltipContent =
    ifo.version === 3.1
      ? t(
          'For the private sale, each eligible participant will be able to commit any amount of CELEBRITY up to the maximum commit limit, which is published along with the IFO voting proposal.',
        )
      : t(
          'For the basic sale, Max CELEBRITY entry is capped by minimum between your average CELEBRITY balance in the IFO CELEBRITY pool, or the pool’s hard cap. To increase the max entry, Stake more CELEBRITY into the IFO CELEBRITY pool',
        )

  const unlimitedToolipContent =
    ifo.version === 3.1
      ? t(
          'For the public sale, Max CELEBRITY entry is capped by your average CELEBRITY balance in the IFO CELEBRITY pool. To increase the max entry, Stake more CELEBRITY into the IFO CELEBRITY pool',
        )
      : t(
          'For the unlimited sale, Max CELEBRITY entry is capped by your average CELEBRITY balance in the IFO CELEBRITY pool. To increase the max entry, Stake more CELEBRITY into the IFO CELEBRITY pool',
        )

  const tooltipContent = poolId === PoolIds.poolBasic ? basicTooltipContent : unlimitedToolipContent

  const { targetRef, tooltip, tooltipVisible } = useTooltip(tooltipContent, { placement: 'bottom-start' })
  const label = isCurrencyCake ? t('Max. CELEBRITY entry') : t('Max. token entry')
    ...
    if (status === 'coming_soon') {
      return (
        <>
          ...
          {ifo[poolId].cakeToBurn !== '$0' && <FooterEntry label={t('CELEBRITY to burn:')} value={ifo[poolId].cakeToBurn} />}
          <FooterEntry
