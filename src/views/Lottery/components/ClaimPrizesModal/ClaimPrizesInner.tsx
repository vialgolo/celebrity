...
const ClaimInnerContainer: React.FC<ClaimInnerProps> = ({ onSuccess, roundsToClaim }) => {
  ...
  const celebrityReward = activeClaimData.cakeTotal
  const dollarReward = celebrityReward.times(cakePriceBusd)
  const rewardAsBalance = getBalanceAmount(celebrityReward).toNumber()
    ...
    if (receipt?.status) {
      toastSuccess(
        ...
        <ToastDescriptionWithTx txHash={receipt.transactionHash}>
          {t('Your CELEBRITY prizes for round %lotteryId% have been sent to your wallet', { lotteryId })}
