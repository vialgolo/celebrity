...
const PoolCard: React.FC<{ pool: DeserializedPool; account: string }> = ({ pool, account }) => {
  ...
  const isCakePool = earningToken.symbol === 'CELEBRITY' && stakingToken.symbol === 'CELEBRITY'
    ...
    >
      <PoolCardHeader isStaking={accountHasStakedBalance} isFinished={isFinished && sousId !== 0}>
        <PoolCardHeaderTitle
          ...
          subTitle={isCakePool ? t('Earn CELEBRITY, stake CELEBRITY') : t('Stake %symbol%', { symbol: stakingToken.symbol })}
        />
